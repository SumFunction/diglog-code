...
def main(args):
    ...
    valid_dataloader = DataLoader(valid_dataset,
                                  ...
                                  batch_size=None)
    cnt = 8578
    lr_scheduler = paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=args.lr, T_max=cnt * args.epochs )
    ...
    print("cnt is ",cnt)
    #lr_scheduler = paddle.optimizer.lr.CosineAnnealingDecay(learning_rate=args.lr, T_max=cnt * args.epochs )
    optimizer = paddle.optimizer.Momentum(learning_rate=lr_scheduler, parameters=model.parameters(),weight_decay=args.weight_decay,momentum=0.9)
    step = 0
    total_time = 0.0
    with open("best_ppl.txt",'r') as f:
        best_ppl = float(f.read())
    log_dir = '/home/aistudio/vis'
        ...
        for epoch in range(args.epochs):
            ...
            for inputs in train_dataloader:
                ...
                if step % args.logging_steps == 0:
                    ...
                    total_time = 0.0
                    writer.add_scalar(step=step//args.logging_steps,value=ppl,tag="train/ppl")
                    writer.add_scalar(step=step//args.logging_steps,value=loss,tag="train/loss")
                    writer.add_scalar(step=step//args.logging_steps,value=lr_scheduler.get_lr(),tag="train/lr")
                if step % args.save_steps == 0:
                    ppl = evaluation(model, valid_dataloader)
                    if dist.get_rank() == 0:
                        save_ckpt(model, tokenizer, args.save_dir, step)
                    if ppl < best_ppl:
                        save_ckpt(model, tokenizer, args.save_dir, "best")
                        best_ppl = ppl
                        print("best ppl is ",best_ppl)
                        with open("best_ppl.txt",'w') as f:
                            f.write(str(best_ppl))
                    writer.add_scalar(step=step//args.save_steps,value=ppl,tag="val/ppl")
                batch_start_time = time.time()
@paddle.no_grad()
